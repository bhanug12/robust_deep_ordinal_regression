...
####write estimated matrix eta_n_es 
if ind==0:
	eta_n_es = np.array([[0.998, 0.002, 0.,    0.   ],[0.132 ,0.753, 0.111 ,0.005],[0. ,   0.081 ,0.807, 0.112],[0. ,   0.   , 0.042 ,0.958]])
elif ind==3:
	eta_n_es = np.array([[0.769 ,0.102, 0.129, 0.   ],[0.028 ,0.672, 0.289 ,0.01 ],[0.03,  0.187 ,0.741 ,0.042],[0.012, 0.055, 0.136, 0.796]])





eta_inv_n_es = np.linalg.inv(eta_n_es)
eta_inv_n_es = torch.from_numpy(eta_inv_n_es)
eta_inv_n_es = eta_inv_n_es.to(device)
...
#####################for l - hat 
	count_iter = 0
	irreg = 0
	for epoch in range(int(num_epochs*1.2)):
		model_hat.train()
		running_loss = 0.0
		total_rows = 0
		lrate = args.l_rate[0]
		...
		optimizer = torch.optim.AdamW(model_hat.parameters(), lr=lrate)
		for batch_idx, (features, l) in enumerate(dat_train):
			features = features.to(device)
			l = l.to(device)
			logits, probas = model_hat(features)
			cost = l_hat(logits, l, eta_inv_n_es, num_classes)
			optimizer.zero_grad()
			cost.backward()
			optimizer.step()
			count_iter += 1
		#	params = model_hat.state_dict()
			irreg += reversals(model_hat.state_dict()['linear_l.bias'])
		...
		print('%s \n%s\n' % (ls,s))
		with open(LOGFILE+'_ES_'+str(rseed)+'_', 'a') as f:
			f.write('%s \n%s\n' % (ls,s))
	
		model_hat = model_hat.to(torch.float64)
		model_hat.eval()
		with torch.set_grad_enabled(False):  # save memory during inference
		#	train_mae, train_mse, train_mzo, ex1 = compute_mae_and_mse(model_hat, dat_train,device=device)
			test_mae, test_mse, test_mzo, ex2 = compute_mae_and_mse(model_hat, dat_test,device=device)
...
#########out final results of model_hat	
	final_thresh = reversals(model_hat.state_dict()['linear_l.bias'])
	model_hat = model_hat.to(torch.float64)
	model_hat.eval()
	with torch.set_grad_enabled(False):  # save memory during inference
		train_mae, train_mse, train_mzo, ex1 = compute_mae_and_mse(model_hat, dat_train,device=device)
		...
		with open(LOGFILE+'_ES_'+str(rseed)+'_', 'a') as f:
			f.write('%f\n%f\n%s\n' %(rseed,fold ,s))
		with open(str(ind)+'/all_results_n', 'a') as f:
			f.write(LOGFILE+'_ES_'+str(rseed)+'_'+str(fold)+'\n%f   %f\n%s\n' % (rseed, fold, s))
		
		s = 'Test - MAE/RMSE/MZO/EX:  %.2f/%.2f/%.2f/%.2f \n' % (test_mae, torch.sqrt(test_mse), test_mzo, ex2)
		print(s)
		with open(LOGFILE+'_ES_'+str(rseed)+'_', 'a') as f:
		    f.write('%s\n' % s)
		with open(str(ind)+'/all_results_ES', 'a') as f:
		    f.write('%s\n' % s)
...
#	
	with open(str(ind)+'/model_es'+'_'+str(rseed)+'_'+str(fold)+'.pt', 'wb') as f:
		torch.save(model_hat, f)
#			
#	with open('model_hat'+'_'+str(rseed)+'_'+str(fold)+'.pt', 'wb') as f:
#				torch.save(model_hat, f)
...
#
	with open(str(ind)+'/all_results_of_lhes', 'a') as f:
		f.write('_model_es\n')
...
#	
	...
	with open(str(ind)+'/irrev'+'lhes', 'a') as f:
	        #f.write('_model_es\n')
		f.write('\n %.3f / %.3f ' %(count_iter, irreg ) )
		f.write('\n final thresh correct %.3f' %(final_thresh))
