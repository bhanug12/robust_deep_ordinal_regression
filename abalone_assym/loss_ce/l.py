...
#####################for l - hat 
	...
	for epoch in range(int(num_epochs)):
		...
		optimizer = torch.optim.AdamW(model_b.parameters(), lr=lrate)
		for batch_idx, (features, l) in enumerate(dat_train):
			features = features.to(device)
			l = l.to(device)
			logits, probas = model_b(features)
			cost = l_hat(logits, l, eta_inv, num_classes)
			optimizer.zero_grad()
			cost.backward()
			optimizer.step()
		#	count_iter += 1
		#	params = model_b.state_dict()
	#		irreg += reversals(model_b.state_dict()['lineal_l.bias'])
		...
		print('%s \n%s\n' % (ls,s))
		with open(LOGFILE+'_B_'+str(rseed)+'_', 'a') as f:
			f.write('%s \n%s\n' % (ls,s))
	
		model_b = model_b.to(torch.float64)
		model_b.eval()
		with torch.set_grad_enabled(False):  # save memory during inference
		#	train_mae, train_mse, train_mzo, ex1 = compute_mae_and_mse(model_b, dat_train,device=device)
			test_mae, test_mse, test_mzo, ex2 = compute_mae_and_mse(model_b, dat_test,device=device)
...
#########out final results of model_b	
	...
	with torch.set_grad_enabled(False):  # save memory during inference
		...
		with open(LOGFILE+'_B_'+str(rseed)+'_', 'a') as f:
			f.write('%f\n%f\n%s\n' %(rseed,fold ,s))
		with open(str(ind)+'/all_results_l', 'a') as f:
			f.write(LOGFILE+'_B_'+str(rseed)+'_'+str(fold)+'\n%f   %f\n%s\n' % (rseed, fold, s))
	...
	with open(str(ind)+'/model_b'+'_'+str(rseed)+'_'+str(fold)+'.pt', 'wb') as f:
...
#
	with open(str(ind)+'/all_results_of_l', 'a') as f:
		f.write('_model_b\n')
...
#	with open(str(ind)+'/irrev'+'l', 'a') as f:
#	        f.write('_model\n')
#	        f.write(' %.3f / %.3f ' %(count_iter, irreg ) )
#
